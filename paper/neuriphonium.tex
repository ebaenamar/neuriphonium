...
We use the pretrained LaBraM transformer~\cite{labram}, a large brain model pretrained on 1,500 hours of EEG across 21 channels. The model's CLS token output (200-dimensional) serves as a learned embedding that captures both spectral and temporal patterns.
...
\begin{itemize}[nosep]
  \item \textbf{NeuroLM integration}: Exploring the use of neural language models trained on EEG data to produce richer style embeddings directly from brain activity, bypassing the text prompt intermediate.
